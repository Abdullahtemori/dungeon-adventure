 ...
 *
 * Updated for Iteration 4: added testMonsterHealCannotExceedMaxHP
 * to verify that healing never pushes HP above the monster's maximum.
 *
 * @author Person 1, Tarik Atasoy, Abdullah Temori
 * @version Iteration 4
 */
public class MonsterTest {
    ...
    // ── Heal Cap Test ───────────────────────────────────────────
    @Test
    public void testMonsterHealCannotExceedMaxHP() {
        Ogre o = new Ogre(); // max HP = 200
        for (int i = 0; i < 10_000; i++) {
            o.setHitPoints(195); // near max so any heal risks overflow
            o.heal();
            assertTrue(o.getHitPoints() <= o.getMaxHitPoints(),
                    "HP exceeded max after heal: " + o.getHitPoints());
        }
    }
}
